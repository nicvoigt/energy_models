...
class TD3Agent():
    def __init__(self, input_dims, alpha=0.0005, beta= 0.001 , tau= 0.005,
                 gamma=0.99, update_actor_interval=5, warmup=1000,
                 n_actions=1, max_size=1000000, layer1_size=400,
                 layer2_size=300, batch_size=100, noise=0.1):
        self.gamma = gamma
        self.tau = tau
        self.max_action = float(1)
        self.min_action = float(0)
    ...
    def choose_action(self, state, epsilon):
        ...
        mu_prime = tf.clip_by_value(mu_prime, self.min_action, self.max_action).numpy()[0]
        self.time_step += 1
